...
/** 保险柜台 - 右键打开保险 GUI */
public class InsuranceKioskBlock extends HorizontalBlock {
    public InsuranceKioskBlock(Properties p) { super(p); this.registerDefaultState(this.stateDefinition.any().setValue(FACING, Direction.NORTH)); }
    ...
    @Override public ActionResultType use(BlockState s, World w, BlockPos pos, PlayerEntity pl, Hand h, BlockRayTraceResult hit) {
        if (!w.isClientSide && pl instanceof ServerPlayerEntity) {
            INamedContainerProvider provider = new SimpleNamedContainerProvider(
                (id, inv, p) -> CampusContainers.INSURANCE_CONTAINER.get().create(id, inv, encodeBuf(pos)),
                new TranslationTextComponent("block.campuslife.insurance_kiosk"));
