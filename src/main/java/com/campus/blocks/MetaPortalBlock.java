...
/** 元宇宙传送门 - 右键打开虚拟地产 GUI */
public class MetaPortalBlock extends HorizontalBlock {
    public MetaPortalBlock(Properties p) { super(p); this.registerDefaultState(this.stateDefinition.any().setValue(FACING, Direction.NORTH)); }
    ...
    @Override public ActionResultType use(BlockState s, World w, BlockPos pos, PlayerEntity pl, Hand h, BlockRayTraceResult hit) {
        if (!w.isClientSide && pl instanceof ServerPlayerEntity) {
            INamedContainerProvider provider = new SimpleNamedContainerProvider(
                (id, inv, p) -> CampusContainers.METAVERSE_CONTAINER.get().create(id, inv, encodeBuf(pos)),
                new TranslationTextComponent("block.campuslife.meta_portal"));
