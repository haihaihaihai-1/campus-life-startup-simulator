...
import com.campus.gui.MentorContainer;
...
public class MentorChairBlock extends HorizontalBlock {
    public MentorChairBlock(Properties properties) {
        super(properties);
    ...
    public ActionResultType use(BlockState s, World w, BlockPos p, PlayerEntity pl, Hand h, BlockRayTraceResult r) {
        if (!w.isClientSide && pl instanceof ServerPlayerEntity) {
            INamedContainerProvider provider = new SimpleNamedContainerProvider(
                (id, inv, player) -> new MentorContainer(id, inv, p),
                new StringTextComponent("导师会谈椊"));
