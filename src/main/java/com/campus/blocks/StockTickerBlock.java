...
public class StockTickerBlock extends HorizontalBlock {
    public StockTickerBlock(Properties props) {
        super(props);
    ...
    public ActionResultType use(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockRayTraceResult hit) {
        if (!world.isClientSide && player instanceof ServerPlayerEntity) {
            ...
            INamedContainerProvider provider = new SimpleNamedContainerProvider(
                (id, inv, p) -> CampusContainers.STOCK_CONTAINER.get().create(id, inv,
                    new net.minecraft.network.PacketBuffer(io.netty.buffer.Unpooled.buffer()).writeBlockPos(pos)),
                new TranslationTextComponent("block.campuslife.stock_ticker")
            );
