...
import com.campus.gui.InvestorContainer;
...
public class InvestorOfficeBlock extends HorizontalBlock {
    public InvestorOfficeBlock(Properties properties) {
        super(properties);
    ...
    public ActionResultType use(BlockState s, World w, BlockPos p, PlayerEntity pl, Hand h, BlockRayTraceResult r) {
        if (!w.isClientSide && pl instanceof ServerPlayerEntity) {
            INamedContainerProvider provider = new SimpleNamedContainerProvider(
                (id, inv, player) -> new InvestorContainer(id, inv, p),
                new StringTextComponent("\u6295\u8d44\u4eba\u529e\u516c\u5ba4"));
