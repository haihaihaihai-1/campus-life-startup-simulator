...
/** 广告牌 - 右键打开营销 GUI */
public class AdBillboardBlock extends HorizontalBlock {
    public AdBillboardBlock(Properties p) { super(p); this.registerDefaultState(this.stateDefinition.any().setValue(FACING, Direction.NORTH)); }
    ...
    @Override public ActionResultType use(BlockState s, World w, BlockPos pos, PlayerEntity pl, Hand h, BlockRayTraceResult hit) {
        if (!w.isClientSide && pl instanceof ServerPlayerEntity) {
            INamedContainerProvider provider = new SimpleNamedContainerProvider(
                (id, inv, p) -> CampusContainers.MARKETING_CONTAINER.get().create(id, inv, encodeBuf(pos)),
                new TranslationTextComponent("block.campuslife.ad_billboard"));
