...
import com.campus.gui.CompetitionContainer;
...
public class CompetitionStageBlock extends HorizontalBlock {
    public CompetitionStageBlock(Properties properties) {
        super(properties);
    ...
    public ActionResultType use(BlockState s, World w, BlockPos p, PlayerEntity pl, Hand h, BlockRayTraceResult r) {
        if (!w.isClientSide && pl instanceof ServerPlayerEntity) {
            INamedContainerProvider provider = new SimpleNamedContainerProvider(
                (id, inv, player) -> new CompetitionContainer(id, inv, p),
                new StringTextComponent("竞赛舞台"));
