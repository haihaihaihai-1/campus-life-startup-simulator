...
public class TaxOfficeBlock extends HorizontalBlock {
    public TaxOfficeBlock(Properties props) { super(props); this.registerDefaultState(this.stateDefinition.any().setValue(FACING, Direction.NORTH)); }
    ...
    @Override public ActionResultType use(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockRayTraceResult hit) {
        if (!world.isClientSide && player instanceof ServerPlayerEntity) {
            ...
            INamedContainerProvider provider = new SimpleNamedContainerProvider(
                (id, inv, p) -> CampusContainers.TAX_CONTAINER.get().create(id, inv,
                    new net.minecraft.network.PacketBuffer(io.netty.buffer.Unpooled.buffer()).writeBlockPos(pos)),
                new TranslationTextComponent("block.campuslife.tax_office"));
