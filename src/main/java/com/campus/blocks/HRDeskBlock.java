...
/** HR 招聘台 - 右键打开员工招聘 GUI */
public class HRDeskBlock extends HorizontalBlock {
    public HRDeskBlock(Properties p) { super(p); this.registerDefaultState(this.stateDefinition.any().setValue(FACING, Direction.NORTH)); }
    ...
    @Override public ActionResultType use(BlockState s, World w, BlockPos pos, PlayerEntity pl, Hand h, BlockRayTraceResult hit) {
        if (!w.isClientSide && pl instanceof ServerPlayerEntity) {
            INamedContainerProvider provider = new SimpleNamedContainerProvider(
                (id, inv, p) -> CampusContainers.EMPLOYEE_CONTAINER.get().create(id, inv, encodeBuf(pos)),
                new TranslationTextComponent("block.campuslife.hr_desk"));
